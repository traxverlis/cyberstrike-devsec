...
scan-quick: ## Quick scan — secrets + critical CVEs only (~30s) [TARGET=.]
	@export PATH="$$PATH:$$HOME/.local/bin"; \
	bash scripts/scan.sh --target $(or $(TARGET),.) --mode quick --output $(REPORTS_DIR)

scan-cicd: ## CI/CD scan — JSON output + strict exit codes [TARGET=.]
	@export PATH="$$PATH:$$HOME/.local/bin"; \
	bash scripts/scan.sh --target $(or $(TARGET),.) --mode cicd --output $(REPORTS_DIR)
...
report: check-deps ## Generate consolidated Markdown report  [OUTPUT=./report.md]
	...
	@# Generate report using generate-report.py
	@export PATH="$$PATH:$$HOME/.local/bin"; \
	python3 scripts/generate-report.py \
		--results-dir $(REPORTS_DIR) \
		--output $(or $(OUTPUT),$(REPORTS_DIR)/report.pdf) \
		--level 1 \
		--format pdf 2>&1
...
generate-consent: ## Generate consent document PDF [TARGET_URL=... SCOPE=...]
	...
	python3 scripts/consent/generate-consent.py \
		--target "$(or $(TARGET_URL),https://target.example.com)" \
		--scope "$(or $(SCOPE),/*)" \
		--requestor "$(or $(REQUESTOR),Security Team)" \
		--company "$(or $(COMPANY),Your Company)" \
		--tester "$(or $(TESTER),Red Team)" \
		--duration "$(or $(DURATION),$(shell date +%Y-%m-%d) to $(shell date -d '+7 days' +%Y-%m-%d 2>/dev/null || date -v+7d +%Y-%m-%d))" \
		--test-types "$(or $(TEST_TYPES),recon,headers,cors,ssl,nikto,nmap)" \
		--exclusions "$(or $(EXCLUSIONS),none)" \
